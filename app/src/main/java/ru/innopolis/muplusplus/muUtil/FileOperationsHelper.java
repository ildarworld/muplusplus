...
import ru.innopolis.muplusplus.callbacks.CancelOperationCallback;
import ru.innopolis.muplusplus.callbacks.ActionCallback;
import ru.innopolis.muplusplus.model.FileArrayEntry;
import ru.innopolis.muplusplus.workers.Trasher;
import ru.innopolis.muplusplus.workers.Unzipper;
import ru.innopolis.muplusplus.workers.Zipper;
...
public class FileOperationsHelper {
	...
	protected static final String TAG = FileOperationsHelper.class.getName();

	
	public static void copyFile(File file, FilesArrayActivity mContext)
	{
		Debug.startMethodTracing("copyFile");
	...
	public static void showProperties(final FileArrayEntry file, final FilesArrayActivity mContext)
	{
		if(mContext!=null)
		new Builder(mContext)
		.setTitle(mContext.getString(R.string.properties_for, file.getName()))
		.setItems(MuUtil.getFileProperties(file, mContext), new OnClickListener() {
	...
	public static void deleteFile(final File file, final FilesArrayActivity mContext, final ActionCallback<Void> callback)
	{
	...
	public static int[] getContextMenuOptions(File file, FilesArrayActivity caller) {

		SettingsHelper prefs = new SettingsHelper(caller);
		
		if(MuUtil.isProtected(file))
		{
			return new int[]{};
		}
		if(MuUtil.isSdCard(file))
		{
		...
		else if(MuUtil.isUnzippable(file))
		{
			if(prefs.isZipEnabled())
			{
	...
	public static void rename(final File file, final FilesArrayActivity mContext, final ActionCallback<Void> callback)
	{
	...
	public static void zip(final File file, final FilesArrayActivity mContext)
	{
		try
		{
			final File zipLoc = new SettingsHelper(mContext).getZipDestinationDir();
	...
	private static void promptZipFileName(final File file,
                                          final FilesArrayActivity mContext, final File zipLoc) {
		final EditText input = new EditText(mContext);
	...
	public static void unzip(final FilesArrayActivity mContext, final List<File> zipFiles, final CancelOperationCallback callback)
	{
		final EditText input = new EditText(mContext);
	...
	public static void doOperation(FileArrayEntry entry, int action, FilesArrayActivity mContext, ActionCallback<Void> callback) {
